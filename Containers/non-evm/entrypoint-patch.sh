...
validate_and_process_file() {
    ...
    if ! validate_contract_file "$file"; then
        log_enhanced "ERROR" "File validation failed for $filename"
        return 1
    fi
    ...
    log_enhanced "SUCCESS" "File $filename passed security validation"
    return 0
}
EOF_SECURITY

    # Insert security functions into entrypoint.sh
    sed -i '/^log_enhanced() {/i\
# Security validation functions\
validate_contract_file() {\
    local file="$1"; local extension="${file##*.}";\
    if [[ ! -f "$file" || ! -r "$file" ]]; then return 1; fi;\
    local file_size=$(stat -f%z "$file" 2>/dev/null || stat -c%s "$file" 2>/dev/null || echo 0);\
    local max_size=$((10 * 1024 * 1024));\
    if [[ $file_size -gt $max_size ]]; then return 1; fi;\
    case $extension in\
...
\
check_for_secrets() {\
    local file="$1"; local secret_patterns=("private.*key" "secret" "password" "token" "api.*key");\
    for pattern in "${secret_patterns[@]}"; do \
        if grep -qi "$pattern" "$file" 2>/dev/null; then \
            log_enhanced "WARNING" "Potential secret found in $file matching pattern: $pattern"; \
        fi; \
...
apply_error_handling_improvements() {
    ...
    # Replace basic exit behavior
    sed -i 's/^set -e$/set -euo pipefail/' /app/entrypoint.sh
    
    # Enhanced tool error handling and retry
    sed -i '/^log_enhanced() {/i\
# Enhanced error handling\
handle_tool_error() {\
    local tool="$1"; local exit_code="$2"; local file="$3";\
    log_enhanced "ERROR" "$tool failed with exit code $exit_code for $file";\
    case $tool in\
        "cargo-build") log_enhanced "WARNING" "Build failed, skipping tests for $file"; return 1 ;;\
...
\
retry_with_backoff() {\
    local max_attempts="$1"; shift; local cmd=("$@"); local attempt=1;\
    while (( attempt<=max_attempts )); do\
        if "${cmd[@]}"; then return 0; fi;\
        log_enhanced "WARNING" "Attempt $attempt for ${cmd[*]} failed, retrying in $((attempt*2))s";\
        sleep $((attempt*2)); ((attempt++));\
    done;\
    log_enhanced "ERROR" "All $max_attempts attempts failed for: ${cmd[*]}"; return 1;\
}\
' /app/entrypoint.sh

    sed -i 's/cargo build-sbf/if ! cargo build 2>&1; then handle_tool_error "cargo-build" $? "$file"; continue; fi/' /app/entrypoint.sh
    sed -i 's/cargo test/if ! cargo test 2>&1; then handle_tool_error "cargo-test" $? "$file"; fi/' /app/entrypoint.sh
    sed -i 's/cargo audit/if ! retry_with_backoff 3 cargo audit 2>&1; then handle_tool_error "cargo-audit" $? "$file"; fi/' /app/entrypoint.sh
    sed -i 's/cargo tarpaulin/if ! cargo tarpaulin --config \/app\/tarpaulin.toml 2>&1; then handle_tool_error "tarpaulin" $? "$file"; fi/' /app/entrypoint.sh
}
...
process_contracts_parallel() {
    ...
    local job_count=0
    local pids=()
    log_enhanced "INFO" "Starting parallel processing with max $max_jobs jobs"
    for file in "$watch_dir"/*.rs; do
        [[ -f "$file" ]] || continue
        if (( job_count>=max_jobs )); then wait -n; ((job_count--)); fi
        process_single_contract "$file" & pids+=($!); ((job_count++))
        log_enhanced "INFO" "Started $(basename "$file") (PID ${pids[-1]})"
    done
    for pid in "${pids[@]}"; do wait "$pid" || log_enhanced "WARNING" "Job $pid failed"; done
    log_enhanced "SUCCESS" "All parallel jobs completed"
}
EOF_PARALLEL

    sed -i '/^# Main processing loop/i\'"$(cat /tmp/patch_parallel)"'' /app/entrypoint.sh
...
monitor_resources() {
    ...
    local disk=$(df /app | awk 'NR==2 {print $5}' | sed 's/%//')
...
EOF_RESOURCE
    ...
    sed -i '/^log_enhanced() {/i\'"$(cat /tmp/patch_resource)"'' /app/entrypoint.sh
    sed -i '/while read path action file; do/a\        if (( $(date +%s) % 10 == 0 )); then monitor_resources; fi' /app/entrypoint.sh
}
...
TIMESTAMP=$(date +%s)
if ! pgrep -f "entrypoint.sh" > /dev/null; then echo "ERROR: Main entrypoint not running" >"$HEALTH_FILE"; exit 1; fi
if [[ ! -d "$WATCH_DIR" ]]; then echo "ERROR: Watch dir inaccessible" >"$HEALTH_FILE"; exit 1; fi
for t in cargo rustc; do command -v "$t" > /dev/null || { echo "ERROR: $t missing" >"$HEALTH_FILE"; exit 1; }; done
disk=$(df /app | awk 'NR==2{print $5+0}'); (( disk>95 )) && { echo "ERROR: Disk usage too high: ${disk}%" >"$HEALTH_FILE"; exit 1; }
echo "OK: Container healthy at $(date)" >"$HEALTH_FILE"; exit 0
EOF_HEALTH

    chmod +x /tmp/health_check.sh && mv /tmp/health_check.sh /app/health_check.sh

    cat << 'EOF_METRICS' > /tmp/patch_metrics
# Metrics collection
collect_metrics() {
    local mf="/tmp/container_metrics.json"
    local ts=$(date +%s)
    ...
    cat << JSON >"$mf"
{"timestamp":$ts,"container":"non-evm","metrics":{"processed_files":$pf,"memory_usage_mb":$mu,"cpu_usage_percent":$cu,"disk_usage_percent":$du,"uptime_seconds":$((ts-START_TIME))}}
JSON
    log_enhanced "INFO" "Metrics collected: $pf files"
}
EOF_METRICS

    sed -i '/^log_enhanced() {/i\'"$(cat /tmp/patch_metrics)"'' /app/entrypoint.sh
    sed -i '/while read path action file; do/a\        if (( $(date +%s) % 60 == 0 )); then collect_metrics; fi' /app/entrypoint.sh
    sed -i '1a\START_TIME=$(date +%s)' /app/entrypoint.sh
}
...
setup_directory_watch() {
    local watch_dir="$1"; local max_retries=3; local retry=0
    while (( retry<max_retries )); do
        if inotifywait -m -e close_write,moved_to,create "$watch_dir" 2>/dev/null | \
           while read path action file; do
               [[ "$file" =~ \.rs$ ]] || continue
               log_enhanced "INFO" "Detected change in $file"
            ...
            log_enhanced "SUCCESS" "Directory watch setup successful"; return 0
        fi
        ((retry++))
    ...
    while true; do
        ...
        monitor_resources; collect_metrics; sleep 10
    done
}
EOF_FALLBACK
