...
check_for_secrets() {
    ...
    for pattern in "${secret_patterns[@]}"; do
        if grep -qi "$pattern" "$file" 2>/dev/null; then
            log_enhanced "WARNING" "Potential secret in $file matching: $pattern"
        fi
    done
}
...
handle_tool_error() {
    ...
    case $tool in
        ...
        "cargo-audit") log_enhanced "WARNING" "Audit failed, review dependencies" ;;
        "tarpaulin")   log_enhanced "WARNING" "Coverage analysis failed, skipping" ;;
        *)             log_enhanced "WARNING" "Unknown tool error, continuing" ;;
    esac
    return 0
}
...
retry_with_backoff() {
    ...
    while (( attempt <= max_attempts )); do
        ...
        log_enhanced "WARNING" "Attempt $attempt failed for: ${cmd[*]}; retry in $((attempt*2))s"
        sleep $((attempt*2))
        ((attempt++))
    done
...
process_single_contract() {
    ...
    if ! cargo build --release 2>&1; then
        handle_tool_error "cargo-build" $? "$filename"
    fi
    if ! cargo test 2>&1; then
        handle_tool_error "cargo-test" $? "$filename"
    fi
    if ! retry_with_backoff 3 cargo audit 2>&1; then
        handle_tool_error "cargo-audit" $? "$filename"
    fi
    if ! cargo tarpaulin --config /app/tarpaulin.toml 2>&1; then
        handle_tool_error "tarpaulin" $? "$filename"
    fi
    log_enhanced "SUCCESS" "Finished $filename in $(( $(date +%s) - start_ts ))s"
    return 0
}
...
process_contracts_parallel() {
    ...
    for file in "$WATCH_DIR"/*.rs; do
        ...
        log_enhanced "INFO" "Spawned $(basename "$file") (PID ${pids[-1]})"
    done
    for pid in "${pids[@]}"; do
...
monitor_resources() {
    ...
    disk=$(df /app | awk 'NR==2{print $5}')
    log_enhanced "INFO" "Resources - Mem: $mem, CPU: $cpu, Disk: $disk"
    (( ${mem%\%} > 90 )) && log_enhanced "WARNING" "High memory usage: $mem"
    (( disk+0 > 90 )) && log_enhanced "WARNING" "High disk usage: $disk"
}
...
collect_metrics() {
    ...
    local ts processed_files mem cpu disk
    ts=$(date +%s)
    processed_files=$(ls -1 /app/processed 2>/dev/null | wc -l)
...
EOF
    log_enhanced "INFO" "Metrics collected: $processed_files files processed"
}

create_health_check() {
    cat <<'EOF' > /app/health_check.sh
#!/bin/bash
HEALTH_FILE="/tmp/container_health"
date +%s > /tmp/health_ts
if ! pgrep -f "entrypoint.sh" > /dev/null; then
  echo "ERROR: Main process not running" > "$HEALTH_FILE"; exit 1
fi
if [[ ! -d "${WATCH_DIR:-/app}" ]]; then
  echo "ERROR: Watch dir inaccessible" > "$HEALTH_FILE"; exit 1
fi
for tool in cargo rustc; do
  command -v "$tool" > /dev/null || { echo "ERROR: $tool missing" > "$HEALTH_FILE"; exit 1; }
done
disk=$(df /app | awk 'NR==2{print $5+0}')
(( disk > 95 )) && { echo "ERROR: Disk usage $disk% too high" > "$HEALTH_FILE"; exit 1; }
...
setup_directory_watch() {
    local dir="$WATCH_DIR" retry=0 max=3
    while (( retry < max )); do
        if inotifywait -m -e close_write,create,moved_to "$dir" 2>/dev/null | \
           while read path action file; do
             [[ "$file" =~ \.rs$ ]] || continue
             log_enhanced "INFO" "Detected $file"
             process_single_contract "$path$file"
           done; then
            return 0
        fi
        ((retry++))
        log_enhanced "WARNING" "inotify attempt $retry failed; retrying..."
        sleep 5
    done
    log_enhanced "ERROR" "inotify failed; switching to polling fallback"
    while true; do
        for f in "$dir"/*.rs; do
            [[ -f "$f" ]] || continue
            local mark="/app/processed/$(basename "$f")"
            if [[ ! -f "$mark" ]]; then
                log_enhanced "INFO" "New file: $(basename "$f")"
                process_single_contract "$f" && { mkdir -p /app/processed; touch "$mark"; }
            fi
        done
        monitor_resources
        collect_metrics
...
log_enhanced "INFO" "Setting up directory watch on $WATCH_DIR"
setup_directory_watch
...
rm -f /tmp/patch_* /tmp/enhanced_fallback

if bash -n /app/entrypoint.sh; then
    log_enhanced "SUCCESS" "Entrypoint.sh syntax valid"
else
    log_enhanced "ERROR" "Entrypoint.sh syntax error"; exit 1
fi

log_enhanced "SUCCESS" "Comprehensive improvements applied!"
exit 0
            ;;
        *)
            log_enhanced "WARNING" "Unknown file extension: .$extension"
